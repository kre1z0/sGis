import {Control, ControlParams, DrawingFinishEvent} from "./Control";
...
export interface PointControlParams extends ControlParams {
    symbol?: Symbol<PointFeature>;
}

/**
 * Control for creating point visualObjects. When active, any click on the map will create a new point feature and add it
...
export class PointControl extends Control {
     ...
     */
    constructor(map: Map, {activeLayer = null, snappingProvider = null, isActive = false, symbol = new PointSymbol()}: PointControlParams = {}) {
    ...
    private _handleClick(event: sGisClickEvent): void {
        ...
        let visualObject = new PointObject(this._snap(event.point.position, event.browserEvent.altKey), {crs: this.map.crs, symbol: this.symbol});

        if (this.activeLayer) this.activeLayer.add(visualObject);
        this.fire(new DrawingFinishEvent(visualObject.feature, event.browserEvent));
    }
